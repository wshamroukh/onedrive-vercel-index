...
// --- 关键修复：使用 dynamic 导入所有预览组件并禁用 SSR ---
...
const FileListing: FC<{ query?: ParsedUrlQuery }> = ({ query }) => {
  ...
  if (responses.length > 0 && 'folder' in responses[0]) {
    ...
    const genTotalSelected = (selected: { [key: string]: boolean }) => {
      ...
      const selectInfo = files.map(c => Boolean(selected[c.id]))
    ...
    const toggleItemSelected = (id: string) => {
      setSelected(prev => {
        ...
        setTotalSelected(genTotalSelected(next))
        return next
      })
    }
    ...
    const toggleTotalSelected = () => {
      if (totalSelected === 2) {
        ...
        const next = Object.fromEntries(getFiles().map(c => [c.id, true]))
        setSelected(next)
        setTotalSelected(2)
      }
    }
    ...
    const handleFolderDownload = (path: string, id: string, name?: string) => () => {
      const files = (async function* () {
        for await (const { meta: c, path: p, isFolder, error } of traverseFolder(path)) {
          if (error) {
            toast.error(t('Failed to download folder'), { duration: 3000 })
            continue
          }
        ...
        {!onlyOnePage && (
          <div className="rounded-b bg-white dark:bg-gray-900 dark:text-gray-100">
            <div className="border-b border-gray-200 p-3 text-center font-mono text-sm text-gray-400 dark:border-gray-700">
              {t('- showing {{count}} page(s) ', { count: size }) +
                (isLoadingMore ? t('of {{count}} file(s) -', { count: folderChildren.length, context: 'loading' }) : t('of {{count}} file(s) -', { count: folderChildren.length, context: 'loaded' }))}
            </div>
            <button
  ...
  if (responses.length > 0 && 'file' in responses[0]) {
